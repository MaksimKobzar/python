...
def get_settings(args, settings):
    ...
    for opt_key, arg in opts:
        if opt_key == '-h':
            ...
            settings['path'] = arg if path.isabs(arg) else path.abspath(arg)
        elif opt_key == '-n':
            settings['name'] = arg
        elif opt_key == '-s':
...
def check_settings(settings):
    ...
    if not path.exists(settings['path']):
        ...
        if ans == "y":
            ...
            if not settings['silent']: print("Path is created: %s") % (settings['path'])
        else:
...
def create_table(settings):
    ...
    with open(temp, 'w+') as table:
        table.write(
            '----------------------------------------------------------------------------------------------------------------------------------------------------------|\n')
        table.write(
            '- ARG NAME                | YOUR CHOICE                          | POSSIBLE VALUES, EXAMPLES           | COMMENTS                                         |\n')
        table.write(
            '----------------------------------------------------------------------------------------------------------------------------------------------------------|\n')
        table.write(
            '| author_name             | Maksim Kobzar (maksim.kobzar@sk.com) | -                                   | It affect some Makefile configurations           |\n')
        table.write(
            '| uvm_version             | uvm1.2                               | uvm1.2, uvm1.1d                     |                                                  |\n')
        table.write(
            '| tb_tag                  | smc_llc                              | smc_rm                              |                                                  |\n')
        table.write(
            '| script_project_dir_path | $(CDVE_PATH)/script/project/Atomos   | $(CDVE_PATH)/script/project/AtomosC |                                                  |\n')
        table.write(
            '| ip_dir_path             | $(PROJ)/design/llc                   | $(PROJ)/design/rm                   |                                                  |\n')
        table.write(
            '| dut_filelist_name       | llc_wrapper_dut                      | rm_dut                              |                                                  |\n')
        table.write(
            '| dut_toplevel_name       | llc_wrapper.f                        | rm_top.f                            |                                                  |\n')
        table.write(
            '| has_reg_model           | y                                    | y, n                                |                                                  |\n')
        table.write(
            '| reg_model_class_name    | csr_block_llc                        | csr_rm_block_reg                    | If has reg_model                                 |\n')
        table.write(
            '| uvm_reg_addr_width      | 32                                   | 1-128                               | If has reg_model                                 |\n')
        table.write(
            '| uvm_reg_data_width      | 32                                   | 1-8196                              | If has reg_model                                 |\n')
        table.write(
            '| submit_run_dir          | y                                    | y, n                                |                                                  |\n')
        table.write(
            '| mini_run_dir            | y                                    | y, n                                |                                                  |\n')
        table.write(
            '| nightly_run_dir         | y                                    | y, n                                |                                                  |\n')
        table.write(
            '| cdve_comps              | n                                    | y, n                                | If "y" is set it will be left some special marks |\n')
        table.write(
            '----------------------------------------------------------------------------------------------------------------------------------------------------------|\n')
